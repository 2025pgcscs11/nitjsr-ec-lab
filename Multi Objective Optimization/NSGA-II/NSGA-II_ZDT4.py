...
GENERATIONS = 200
DIMENSION = 10
LOWER_BOUND = np.array([0.0]  + [-5.0] * (DIMENSION - 1))
UPPER_BOUND = np.array([1.0]  + [ 5.0] * (DIMENSION - 1))
CROSSOVER_RATE = 0.9
MUTATION_RATE = 0.1
...
def fitness(chromosome):
    ...
    # g(x)
    g = 1 + 10 * (n - 1) + np.sum(chromosome[1:] ** 2 - 10 * np.cos(4 * np.pi * chromosome[1:]))
...
def mutate(chromosome):
    ...
    if np.random.rand() >= MUTATION_RATE:
        ...
        for i in range(len(chromosome)):
            ...
            # Apply mutation
            chromosome[i] = chromosome[i] + delta * (UPPER_BOUND[i] - LOWER_BOUND[i])
