...
def generate_initial_population(params):
    ...
    for _ in range(POP_SIZE):
        ...
        chromosome = np.concatenate([number_of_validators, T_bits])
...
def solve_gap_file(filename):
    ...
    for idx, instance in enumerate(instances, start=1):
       ...
       # latency min, max calculation
        ...
        plt.title(f"CONVERGENCE GRAPH || BINARY CODED GENETIC ALGORITHM || BLOCKCHAIN CONFIGURATION OPTIMIZATION || {os.path.splitext(os.path.basename(filename))[0]} || Setting {idx}")
        plt.legend(loc='best')
