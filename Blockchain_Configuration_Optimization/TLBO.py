...
# ==========================================================
def teaching_learning_based_optimization(params):
       # Initialize random population
    population = generate_initial_population(params)

    # Evaluate fitness
    fitness_values = np.array([
        fitness(population[i], params)
    ...
    for t in range(ITERATIONS):
        ...
        for i in range(POP_SIZE):
            ...
            x_p_index = np.random.choice([j for j in range(POP_SIZE) if j != i])
    ...
    for idx, instance in enumerate(instances, start=1):
       ...
       # latency min, max calculation
        ...
        for run in range(num_runs):
            ...
            best_assignment, best_utility, fitness_per_gen = teaching_learning_based_optimization(params)
        ...
        plt.title(f"CONVERGENCE GRAPH || TECHING LEARNING BASED OPTIMIZATION || BLOCKCHAIN CONFIGURATION OPTIMIZATION || {os.path.splitext(os.path.basename(filename))[0]} || Setting {idx}")
        plt.legend(loc='best')
