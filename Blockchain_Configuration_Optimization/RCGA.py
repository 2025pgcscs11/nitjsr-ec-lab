...
def generate_initial_population(params):
    ...
    for _ in range(POP_SIZE):
        ...
        chromosome = np.concatenate([validators, [T]])
...
def decode_chromosome(chromosome, params):
    ...
    v = chromosome[:params["N"]]
    T = int(round(chromosome[-1]))
...
def crossover(p1, p2, params):
    ...
    # Validators
    child1[:N] = np.clip(child1[:N], 0, 1)
    child2[:N] = np.clip(child2[:N], 0, 1)

    # Transactions
    child1[N] = np.clip(child1[N], params["t_min"], params["t_max"])
    child2[N] = np.clip(child2[N], params["t_min"], params["t_max"])
...
def mutate(chromosome, params):
    ...
    for i in range(len(chromosome)):
        ...
        # Apply mutation per gene (important!)
        if np.random.rand() < MUTATION_RATE:
            ...
            chromosome[i] = chromosome[i] + delta * (upper - lower)

    # Apply bounds separately
    chromosome[:N] = np.clip(chromosome[:N], 0.0, 1.0)
    chromosome[N] = np.clip(chromosome[N], t_min, t_max)
...
# ==========================================================
def real_coded_genetic_algorithm(params):
    ...
    for gen in range(GENERATIONS):
        ...
        for i in range(len(offspring_population)):
            offspring_population[i] = mutate(offspring_population[i], params)
    ...
    for idx, instance in enumerate(instances, start=1):
       ...
       # latency min, max calculation
        ...
        for run in range(num_runs):
            ...
            best_assignment, best_utility, fitness_per_gen = real_coded_genetic_algorithm(params)
        ...
        plt.title(f"CONVERGENCE PLOT || RCGA || {os.path.splitext(os.path.basename(filename))[0]} || Instance {idx}")
        plt.legend(loc='best')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        os.makedirs("plots", exist_ok=True)
        plt.savefig(f"plots/{os.path.splitext(os.path.basename(filename))[0]}_instance_{idx}_RCGA_penalty_convergence.png", dpi=300)
        plt.show()
