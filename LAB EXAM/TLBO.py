...
def solve_square_function():
    ...
    for run in range(num_runs):
        ...
        print(f"Run {run+1}: Best Value = {best_value:}, Time = {run_time:} sec")
    ...
    plt.ylabel("Best Fitness Value (Min)")
    plt.title("TLBO Convergence (Rosenbrock Function)")
    plt.legend(loc='best')
...
if __name__ == "__main__":
    ...
    for result in all_results:
        ...
        print("\n===== FINAL SUMMARY (Rosenbrock FUNCTION) =====")
        ...
        print(f"\nAverage time/run    : {np.mean(times):} sec")
        print(f"Total execution time  : {np.sum(times):} sec")