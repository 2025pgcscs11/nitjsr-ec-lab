...
def repair_student_greedy_cost(student, C, R, B):
    """
    Repairs an infeasible student for the MAXIMIZATION GAP problem.
    ...
    agents = np.round(student).astype(int)
        ...
        for a in range(m):
            ...
            while resource_used[a] > B[a]:
                ...
                if best_move is not None:
                    ...
                    # (student is irreparable for this agent)
                    break
...
def solve_gap_file(filename):
    ...
    for idx, (C, R, B) in enumerate(instances, start=1):
        ...
        plt.title(f"CONVERGENCE GRAPH || TEACHER-LEARNER BASED OPTIMIZATION ALGORITHM || REPAIR BASED (GREEDY || {os.path.splitext(os.path.basename(filename))[0]} || Instance {idx}")
        plt.legend(loc='best')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        # os.makedirs("plots", exist_ok=True)
        # plt.savefig(f"plots/{os.path.splitext(os.path.basename(filename))[0]}_instance_{idx}_TLBO_repair_cost_convergence.png", dpi=300)
        plt.show()
