...
def solve_gap_file(filename):
    ...
    for idx, (C, R, B) in enumerate(instances, start=1):
        ...
        plt.title(f"CONVERGENCE PLOT || RCGA(repair based using random) || {os.path.splitext(os.path.basename(filename))[0]} || Instance {idx}")
        plt.legend(loc='best')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        os.makedirs("plots", exist_ok=True)
        plt.savefig(f"plots/{os.path.splitext(os.path.basename(filename))[0]}_instance_{idx}_RCGA_repair_random_convergence.png", dpi=300)
        plt.show()
