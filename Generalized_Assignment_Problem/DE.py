...
def differential_evolution_based_optimization(C, R, B):
    ...
    for t in range(ITERATIONS):
        ...
        # Bound and Evaluate the fitness
        for i in range(POP_SIZE):
            for var in range(len(trial_vector[i])):
                trial_vector[i][var] = np.clip(np.round(var),0 ,m - 1)
...
if __name__ == "__main__":
    solve_multiple_files(files)
>>>>>>> 8d4346ff82d543de1ab92f23fc47f64c9c00e083
