...
# ==========================================================
def objective_function(student, C, R, B, penalty_weight=1000):
    cost = 0
    penalty = 0
...
def artificial_bee_colony_opimization(C, R, B):
    ...
    objective_function_values = np.array([
        objective_function(population[i], C, R, B)
        for i in range(POP_SIZE)
    ])

    # Evaluate fitness of the population
    fitness_values = np.array([
        fitness(objective_function_values[i])
        for i in range(POP_SIZE)
    ])

    # Initial trial vector of the popultion
    trial_vector = np.zeros(POP_SIZE,1)
